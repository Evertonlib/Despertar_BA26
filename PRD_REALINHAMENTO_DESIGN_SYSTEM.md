...
### 8.1 Riscos do modelo de publicação (atenção especial)

O `Despertar_BA26` publica **o repositório inteiro, direto na raiz do GitHub Pages** — não existe pasta `dist/`, não existe etapa de build, não existe ambiente de prévia. Isso é diferente do `Painel-treino`, que passa por um build Vite e publica só a pasta `dist/` gerada. A consequência prática: **todo commit vai direto pro ar, exatamente como está no repositório**, e não há nenhum passo intermediário (compilação, lint, empacotamento) capaz de barrar um erro antes do público. Os riscos abaixo derivam todos desse fato e são os mais graves desta melhoria.

1. **Erro de sintaxe em HTML/CSS/JS derruba o app publicado na hora.** Como todo o app vive num único `index.html` com `<style>` e `<script>` inline, uma chave não fechada no CSS, uma vírgula sobrando no objeto de cores da nova `sc()` ou uma template string mal fechada no HTML da Régua de Zona não geram erro em lugar nenhum antes de publicar — geram uma tela quebrada (ou em branco) para quem abrir o app. Esta melhoria mexe exatamente nos três: reescreve o bloco `<style>` inteiro, reescreve uma função JS e injeta HTML novo no hero.
   *Mitigação:* abrir o `index.html` local no navegador e percorrer os dois modos (com ciclo carregado e sem ciclo) **antes** do commit, com o console aberto e sem nenhum erro; fazer a mudança em **um único commit isolado e reversível**, sem misturar com outras alterações, para que um `git revert` do commit restaure o app publicado em um passo.

2. **Caminho de fonte quebrado só se manifesta depois de publicado.** O ambiente local é Windows (sistema de arquivos indiferente a maiúsculas/minúsculas); o GitHub Pages diferencia. Um `@font-face` apontando para `fonts/Inter-Regular.woff2` enquanto o arquivo commitado se chama `fonts/inter-regular.woff2` **funciona perfeitamente na validação local e dá 404 no ar**. Vale o mesmo para caminho relativo escrito de forma diferente do padrão já usado no projeto (`./arquivo`, como no `sw.js`).
   *Mitigação:* padronizar todos os nomes de arquivo de fonte em minúsculas, escrever os caminhos no `@font-face` exatamente com o mesmo case do arquivo commitado, e conferir a aba Network do navegador (nenhum 404) na primeira abertura após a publicação.

3. **Um único `.woff2` faltando derruba o offline inteiro do app.** O `sw.js` instala o cache com `cache.addAll(ASSETS)`, que é atômico: se **qualquer** item da lista responder 404, a promessa falha, o service worker não instala e o app deixa de funcionar sem internet — um efeito colateral desproporcional a "uma fonte não baixou". Como esta melhoria acrescenta vários arquivos novos à lista `ASSETS`, o risco de um item errado na lista é real.
   *Mitigação:* conferir que cada caminho listado em `ASSETS` corresponde a um arquivo efetivamente commitado (o Git ignora arquivos vazios/não adicionados sem avisar); em caso de dúvida sobre um peso de fonte, é preferível não listá-lo em `ASSETS` (ele ainda será buscado da rede) a listar um caminho que pode não existir.

4. **MIME type e integridade dos binários de fonte.** O GitHub Pages serve `.woff2` como `font/woff2` normalmente, mas arquivos de fonte são binários e podem ser corrompidos por um commit feito com normalização de fim de linha ativa. Um `.woff2` corrompido é servido com status 200 e simplesmente não renderiza.
   *Mitigação:* garantir que os arquivos de fonte sejam tratados como binários pelo Git (sem conversão de CRLF) e validar visualmente, após publicar, que os três tipos de letra realmente aparecem — e não o fallback de sistema.

5. **Não há como voltar atrás "só um pouco".** Sem prévia, o único mecanismo de recuperação é reverter o commit e esperar a republicação do Pages (tipicamente em torno de um minuto, mas não instantânea). Durante esse intervalo o app fica no ar quebrado para quem abrir.
   *Mitigação:* publicar em um momento de baixo uso, manter o commit único (ver risco 1) e verificar o app publicado imediatamente após o deploy, não horas depois.

### 8.2 Demais riscos

6. **Peso adicional de download na primeira visita.** Os arquivos `.woff2` aumentam o tamanho baixado no primeiro carregamento do app (antes não havia nenhuma fonte customizada). Mitigação: usar apenas os pesos realmente necessários (os mesmos do `Painel-treino`) e deixar o service worker cachear tudo após o primeiro carregamento, para que aberturas seguintes (inclusive offline) não paguem esse custo de novo.
7. **Cache do service worker servindo uma versão sem as fontes novas.** Se o nome do cache (`CACHE_NAME`) não for incrementado ao publicar, usuárias que já tinham o app instalado continuariam vendo a versão antiga (fontes de sistema, cores antigas) até uma limpeza manual de cache. Mitigação: bumping do nome do cache é parte explícita desta melhoria (seção 3), reaproveitando a lógica de limpeza de caches antigos que o `sw.js` já possui.
8. **Falha ao carregar um arquivo de fonte** (arquivo ausente, corrompido, ou caminho relativo errado — ver riscos 2, 3 e 4 para as causas específicas ligadas à publicação). Mitigação: a pilha de fallback (`sans-serif`/`ui-monospace, monospace`) garante que o texto continue legível mesmo se uma fonte não carregar — o app não trava nem fica com texto invisível.
9. **Estranhamento visual da usuária com a mudança de mapeamento de cor por tipo de treino.** Um treino que hoje aparece, por exemplo, em roxo (`Endurance`) ou ciano (`Z2`/`Z3`) passará a aparecer em outra cor (dourado para Endurance, azul/verde para Z1/Z2, etc.), pois os critérios de cor mudam para seguir o padrão oficial de zona. Mitigação: é uma consequência aceita e desejada desta melhoria (alinhar as cores entre os apps da família), documentada aqui para não ser confundida com um defeito.
